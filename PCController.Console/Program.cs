...
namespace PCController.Console
{
    ...
    public class Program
    {
        ...
        private static async Task CodeExamples(GitHubClient client)
        {
            ...
            System.Console.WriteLine($"Search.SearchCode (Simple Search): TotalCode={result.TotalCount}");
        ...
        private static async Task CodePagingExample(GitHubClient client)
        {
            ...
            for (request.Page = 1; request.Page <= maximumPage; request.Page++)
            {
                ...
                if (request.Page == 1)
                    System.Console.WriteLine($"Search.SearchCode (Paging): TotalCount={result.TotalCount}");

                foreach (SearchCode code in result.Items)
                    System.Console.WriteLine($"  {code.Path}");
